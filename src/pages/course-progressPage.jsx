        ...
        <>
            <div className="flex flex-col h-screen bg-[#1c1d1f] text-white">
                ...
                <div className="flex items-center justify-between p-4 bg-[#1c1d1f] border-b border-gray-700">
                    <Button className="font-bold flex items-center space-x-2" onClick={() => navigate("/my-courses-page")}>
                        <ChevronLeft className="h-5 w-5" />
                        <span className="hidden md:inline">Back To My Courses</span>
                    </Button>
                ...
                <div className="flex flex-1 flex-col lg:flex-row overflow-hidden">
                    <div className={`flex-1 ${isSideBarOpen ? "lg:mr-[400px]" : "mr-0"} transition-all duration-300`}>
                        <VideoPlayer
                            height="100%"
                            width="100%"
                    ...
                    <div
                        className={`fixed top-[68px] right-0 bottom-0 w-full lg:w-[400px] border-l border-gray-700 bg-[#1c1d1f] transform transition-transform duration-300 ${
                            isSideBarOpen ? "translate-x-0" : "translate-x-full"
                        }`}
                    >
                        <Tabs defaultActiveKey="1" className="bg-white text-black font-bold h-full">
                            <Tabs.TabPane tab="Course Content" key="1">
                                <div className="p-4 text-black">
                                    {studentCourseProgress?.CourseDetails?.curriculum?.map((item, index) => (
                                        <div
                                            key={index}
                                            className="mb-2 text-black flex items-center space-x-2 text-sm font-bold cursor-pointer"
                                        >
                ...
                <Modal open={lockCourse} footer={false}>
                    <div className="flex flex-col items-center justify-center p-9 m-5 space-y-5 font-semibold">
                        <h1 className="text-xl font-bold"> You Can't view this page</h1>
                        <p className="text-gray-500 text-md font-semibold">Please Purchase this Course to get access</p>
                    </div>
                </Modal>

                <Modal open={showLectureComplete} footer={false}>
                    <div className="flex flex-col items-center justify-center p-9 m-5 space-y-5 font-semibold">
                        <h1 className="text-xl font-bold"> Congrats!! You Have Completed the Course</h1>
                        <div className="flex gap-5">
                            <Button
                                className="bg-black text-white font-semibold rounded-md"
                                type="primary"
